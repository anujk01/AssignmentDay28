...
namespace AssignmentDay28
{
    ...
    public class UC10 : IAddressBook
    {
        public static Dictionary<string, List<UC10>> City = new Dictionary<string, List<UC10>>();
        public static Dictionary<string, List<UC10>> State = new Dictionary<string, List<UC10>>();
        public List<UC10> stateList;
        public List<UC10> cityList;
        public List<UC10> people;

        public UC10()
        {
            people = new List<UC10>();
        }
        ...
        public UC10(string firstName, string lastName, string address, string city, string state, string zip, string phoneNumber, string email)
        {
        ...
        {
            int contact = 0;
            UC10 person = new UC10(firstName, lastName, phoneNum, address, city, state, zipCode, emailId);
            if (contact == 0)
            {
                ...
                {
                    List<UC10> existing = State[state];
                    existing.Add(person);

                }
                else
                {
                    stateList = new List<UC10>();
                ...
                {
                    List<UC10> existing = City[city];
                    existing.Add(person);

                }
                else
                {
                    cityList = new List<UC10>();
                    cityList.Add(person);
            ...
            {
                UC10 addressBookSystems = people.Find(x => x.firstName.Equals(firstName));
                if (addressBookSystems == null)
                {
                    person = new UC10(firstName, lastName, address, city, state, zipCode, phoneNum, emailId);
                    people.Add(person);
                    if (State.ContainsKey(state))
                    {
                        List<UC10> existing = State[state];
        ...
        public void PrintContact(UC10 person)
        {
        ...
        public void DeletePeople()
        {
            ...
            UC10 person = people.FirstOrDefault(x => x.firstName.ToUpper() == firstName.ToUpper());
            if (person == null)
            {
        ...
        public static void StoreCityList(string key, List<UC10> cityList, string city)
        {
            List<UC10> CityList = cityList.FindAll(a => a.city.ToLower() == city);
            foreach (var i in CityList)
            {
                Console.WriteLine("Found person \"{0}\" in Address Book \"{1}\" , residing in City {2}", i.firstName, key, i.city);
            }
        }
        public static void StoreStateList(string key, List<UC10> stateList, string state)
        {
            List<UC10> StateList = stateList.FindAll(x => x.state.ToLower() == state);
